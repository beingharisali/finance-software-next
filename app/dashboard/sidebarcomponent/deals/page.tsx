...
import { fetchUsers } from "@/services/user.api"; // brokers API
import http from "@/services/http"; // Axios instance for backend
...
const [editingDealId, setEditingDealId] = useState<string | null>(null);
  ...
  const brokerName = brokers.find((b) => b._id === selectedBroker)?.fullname || "";
  const clientName = clientObj ? `${clientObj.firstName} ${clientObj.lastName}` : "";
if (editingDealId) {
  ...
  // Set client
  const clientObj = clients.find(c => `${c.firstName} ${c.lastName}` === deal.client);
...
<div className="mb-3">
  ...
  >
    ...
    {products.map((p) => {
      ...
      const isAssigned = deals.some((d) => d.products.includes(p._id));
